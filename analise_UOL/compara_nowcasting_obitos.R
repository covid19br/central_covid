...
################################################################################
data.dir <- "./analise_UOL/output/nowcasting_estimates_direct/"
datas <- get.data.base(list.files(data.dir), last=FALSE)
f1 <- function(data, data.dir){
  data.lab <- format(data, "%Y_%m_%d")
  dados <-  read.csv(paste0(data.dir,"nowcasting_estimates_direct_", data.lab,".csv"))
  dados$dt_base  <- data
...
ldply(lapply(rev(datas), f1, data.dir = data.dir)) 
  filter(data >= min(rev(datas))) %>% 
  ggplot(aes(x=data)) +
