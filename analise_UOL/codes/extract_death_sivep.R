...
sivep_17_08<-read.sivep.generica(file.name = './dados/SIVEP-Gripe/SRAGHospitalizado_2020_08_17.zip')
sivep_10_08<-read.sivep.generica(file.name = './dados/SIVEP-Gripe/SRAGHospitalizado_2020_08_10.zip')
...
sivep_10_08_obitos<-sivep_10_08 %>%
  ...
  as.data.frame()
write.csv(sivep_10_08_obitos, "~/Área de Trabalho/central_covid/analise_UOL/dados/extract_dates_sivep_10_08.csv", row.names = FALSE)
sivep_10_08_obitos_sum<-sivep_10_08_obitos%>%
  group_by(dt_evoluca)%>%
  dplyr::summarise(N=n())%>%
  mutate(Cum=cumsum(N))%>%
  as.data.frame()
sivep_10_08_sin_sum<-sivep_10_08_obitos%>%
  group_by(dt_sin_pri)%>%
  dplyr::summarise(N=n())%>%
  mutate(Cum=cumsum(N))%>%
  as.data.frame()

write.csv(sivep_10_08_obitos_sum,"~/Área de Trabalho/central_covid/analise_UOL/dados/spreasheet_e_CSV/extract_10_08.csv")
