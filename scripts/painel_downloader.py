...
def find_last_date(output_folder):
    ...
    filesxlsx = glob(output_folder + '/*.xlsx')
    files = filescsv + fileszip + filesxlsx
...
def check_for_new_file(index_page_address, last_date):
    ...
    reg = re.compile(r".*HIST_PAINEL_COVIDBR.*(\d\d/\d\d/\d\d\d\d).*",
...
if __name__ == '__main__':
    ...
    index_page_address = "https://shiny.hmg.saude.gov.br/tr/dataset/painel-da-covid-19/"
    output_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../dados/painel-Brasil')
    gitUpdate = False

    last_date = find_last_date(output_folder)
    newfile = check_for_new_file(index_page_address, last_date)
    if newfile:
        output_fname = "hist_painel_covidbr_{data}.csv".format(data=newfile[0].strftime("%Y_%m_%d"))
        ...
        if gitUpdate:
            os.system('''cd {folder} &&
                   git add {outzip} &&
                   git commit -m "[auto] painel Covid-19 Brasil de {data}" &&
                   rm {outfile} &&
