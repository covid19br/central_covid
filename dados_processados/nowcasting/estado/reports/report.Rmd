---
title: "Análise de dados COVID-19 por Estado"
date: "`r Sys.Date()`"
...
ids <- c(12, 27, 13, 16, 29, 23, 53, 32, 52, 21, 31, 50, 51, 15, 25, 26, 22, 41, 33, 24, 11, 14, 43, 42, 28, 35, 17)
siglas <- c("AC", "AL", "AM", "AP", "BA", "CE", "DF", "ES", "GO", "MA", "MG", "MS", "MT", "PA", "PB", "PE", "PI", "PR", "RJ", "RN", "RO", "RR", "RS", "SC", "SE", "SP", "TO")
nomes <- c("Acre", "Alagoas", "Amazonas", "Amapá", "Bahia", "Ceará", "Distrito Federal", "Espírito Santo", "Goiás", "Maranhão", "Minas Gerais", "Mato Grosso do Sul", "Mato Grosso", "Pará", "Paraíba", "Pernambuco", "Piauí", "Paraná", "Rio de Janeiro", "Rio Grande do Norte", "Rondônia", "Roraima", "Rio Grande do Sul", "Santa Catarina", "Sergipe", "São Paulo", "Tocantins")

for (i in seq(length(siglas))) {
    nome = nomes[i]
    id = ids[i]
    sigla = siglas[i]

    # pula os que não têm gráficos
    if (! file.exists(paste0(output.dir, sigla, '/plots/plot_nowcast_srag.svg')))
        next
    ...
    for (tipo in c("COVID-19", "SRAG")) {
        ...
        for (casos in c("Casos", "Óbitos")) {
            ...
            if (file.exists(paste0(output.dir,sigla,'/plots/plot_nowcast_',ob,tipol,'.svg')))
                cat(str_interp("

### ${casos} ${tipo} Diários

![${casos} ${tipo} diários](${output.dir}${sigla}/plots/plot_nowcast_${ob}${tipol}.svg)
...
"))
            if (file.exists(paste0(output.dir,sigla,'/plots/plot_nowcast_cum_',ob,tipol,'.svg')))
                cat(str_interp("

### ${casos} ${tipo} acumulados e projeção

![${casos} ${tipo} diários](${output.dir}${sigla}/plots/plot_nowcast_cum_${ob}${tipol}.svg)
...
"))
            ...
            #if (file.exists(paste0(output.dir,sigla,'/plots/plot_tempo_dupl_',ob,tipol,'.svg')))
            if(FALSE)  # no TD for now
                cat(str_interp("

### ${casos} ${tipo}: Tempo de duplicação

![Tempo de duplicação de ${casos} ${tipo}](${output.dir}${sigla}/plots/plot_tempo_dupl_${ob}${tipol}.svg)
...
"))
    ...
    if (file.exists(paste0(output.dir,sigla,'/plots/plot_estimate_R0_',tipol,'.svg')))
        cat(str_interp("
### Número de reprodução efetivo (calculado com casos ${tipo})

![Tempo de duplicação de ${casos} ${tipo}](${output.dir}${sigla}/plots/plot_estimate_R0_${tipol}.svg)
...
for (covid in c("covid", "srag")){
    for (drs in list.files(output.dir)){
        data <- list.files(paste0(output.dir, drs, "/tabelas_nowcasting_para_grafico/"), pattern="r_efetivo_covid_.*.csv") %>%
