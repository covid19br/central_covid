...
## Ocupacao diaria de leitos UTI, enfremaria e total
dados.oc <-
    dados_seade %>%
    mutate(pacientes_ultimo_dia = pacientes_enf_ultimo_dia + pacientes_uti_ultimo_dia,
           leitos_dia = total_covid_enf_ultimo_dia + total_covid_uti_ultimo_dia,
           ocupacao_dia = 100*pacientes_ultimo_dia/leitos_dia,
           ocup_enf_dia = 100*pacientes_enf_ultimo_dia/total_covid_enf_ultimo_dia,
           ocup_uti_dia = 100*pacientes_uti_ultimo_dia/total_covid_uti_ultimo_dia)
    
p.oc <- 
    filter(dados.oc, nome_drs=="Município de São Paulo") %>%
    ggplot(aes(datahora)) +
    geom_line(aes(y = ocupacao_dia, colour="Total")) +
    geom_line(aes(y = ocup_enf_dia, colour="Enfermaria")) +
    geom_line(aes(y = ocup_uti_dia, colour="UTI")) +
    ##scale_x_date(date_breaks = "3 month", date_labels = "%b") +
    labs(x="",
