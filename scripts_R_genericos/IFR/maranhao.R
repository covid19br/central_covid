...
## Prevalencias para 01/01/2021, sem correcao soro-reversão
## Para casos confirmados
## Prevalencia de 40.4%, estimada para 08/08/2020
MA.ago <- projeta.inquerito(Npop = Npop.MA,
                            inq.data = as.Date("2020-08-08"),
                            inq.preval = 0.404,
                            data.proj = target.data,
                            lista = lista.MA)
## Prevalencia de 38.1% para 30/10/2020
MA.out <- projeta.inquerito(Npop = Npop.MA,
                            inq.data = as.Date("2020-10-30"),
                            inq.preval = 0.381,
                            data.proj = target.data,
                            lista = lista.MA)
...
## Usando casos SRAG
## Prevalencia de 40.4%, estimada para 08/08/2020
MA.srag.ago <- projeta.inquerito(Npop = Npop.MA,
                                 inq.data = as.Date("2020-08-08"),
                                 inq.preval = 0.404,
                                 data.proj = target.data,
                                 lista = lista.MA.2)
## Prevalencia de 38.1% para 30/10/2020
MA.srag.out <- projeta.inquerito(Npop = Npop.MA,
                                 inq.data = as.Date("2020-10-30"),
                                 inq.preval = 0.381,
                                 data.proj = target.data,
                                 lista = lista.MA.2)
...
kable(tabela.MA[,c("data.preval","prevalencia", "IFR", "prev.proj.IFR" )],
      digits = c(NA,1,3,1),
      col.names = c("Fase","Prevalência %", "IFR %", "Prevalência projetada"))
kable(tabela.MA[,c("data.preval","prevalencia", "IHR", "prev.proj.IHR" )],
      digits = c(NA,1,3,1),
      col.names = c("Fase","Prevalência %", "IHR %", "Prevalência projetada"))

kable(tabela.MA.srag[,c("data.preval","prevalencia", "IFR", "prev.proj.IFR" )],
      digits = c(NA,1,3,1),
      col.names = c("Fase","Prevalência %", "IFR %", "Prevalência projetada"))
kable(tabela.MA.srag[,c("data.preval","prevalencia", "IHR", "prev.proj.IHR" )],
      digits = c(NA,1,3,1),
      col.names = c("Fase","Prevalência %", "IHR %", "Prevalência projetada"))
